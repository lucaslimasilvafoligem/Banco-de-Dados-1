...
CREATE OR REPLACE PROCEDURE ajusta_preco AS
  CODIGO_CAT NUMBER;
BEGIN
  UPDATE PRODUTO
  SET PRODUTO.PRECO_VENDA = PRODUTO.PRECO_VENDA + 100
  WHERE PRODUTO.COD_CATEGORIA = CODIGO_CAT;

  UPDATE PRODUTO
  SET PRODUTO.PRECO_COMPRA = PRODUTO.PRECO_COMPRA + 100
  WHERE PRODUTO.COD_CATEGORIA = CODIGO_CAT;
...
CREATE OR REPLACE FUNCTION calcula_total_fornecedor(codigo_fornecedor INTEGER) RETURN NUMBER IS
    valor_total_fornecimentos NUMBER := 0;
BEGIN 
    SELECT SUM(p.preco_compra * cp.quantidade) INTO valor_total_fornecimentos
    ...
    JOIN compra_produto cp on p.codprod = cp.codigo_produto
    WHERE fncd.codforn = codigo_fornecedor;
...
CREATE OR REPLACE TRIGGER Modifica_Nome_Transportadora
...
create or replace TRIGGER ATUALIZA_END_BAIRRO
